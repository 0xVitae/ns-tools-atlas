...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (hostname !== "ns.com" && !hostname.endsWith(".ns.com")) {
    return res
      .status(400)
      .json({ valid: false, error: "URL must be from ns.com" });
  }
  ...
  try {
    ...
    try {
      response = await fetch(url, {
        ...
        headers: {
          "User-Agent": "NS-tool-Atlas/1.0 (Profile Validator)",
        },
        redirect: "follow",
      });
    } catch (headError) {
    ...
    if (response.ok) {
      ...
      return res
        .status(200)
        .json({ valid: false, error: `Server returned ${response.status}` });
    }
  } catch (error) {
    ...
    return res
      .status(200)
      .json({ valid: false, error: "Could not verify profile" });
  }
}
